...
def deslizar_para_cima(matriz):
    ...
    for coluna in range(len(matriz[0])):
        ...
        # Percorre a coluna novamente e substitui os valores pelo próximo valor não nulo da lista, ou zero se não houver mais valores não nulos
        for linha in range(len(matriz)):
            if linha < len(valores_nao_nulos):
                matriz[linha][coluna] = valores_nao_nulos[linha]
            else:
                matriz[linha][coluna] = 0
    # Retorna a matriz deslizada para cima
    return matriz
...
for i in deslizar_para_baixo(tab3):
    print(i)
